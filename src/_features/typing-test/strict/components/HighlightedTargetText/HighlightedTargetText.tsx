import './styles.css'
import { InitialState, Statistics } from "@/_features/typing-test/strict/types";
...
const HighlightedTargetText = ({ targetText, statistics, initialState, setInitialState, setStatistics, isStarted, isFetchingTargetText, children, timeRemain, setTimeRemain, inputBoxCaretPos }: HighlightedTargetTextProps) => {
  ...
  useEffect(() => {
    if (targetText !== "") {
      let currentRun: number[] = []; // indexes of current uppercase run
      const collected: number[] = []; // final indexes to store

      for (let i = 0; i < targetText.length; i++) {
        const char = targetText[i];

        if (char === char.toUpperCase() && /^[A-Za-z]$/.test(char)) {
          currentRun.push(i); // add index to current run
        } else {
          if (currentRun.length >= 3) {
            collected.push(...currentRun); // only store runs >= 3
          }
          currentRun = []; // reset for next run
        }
      }

      // handle leftover run at end of string
      if (currentRun.length >= 3) {
        collected.push(...currentRun);
      }

      // update the statistics state
      setStatistics(draft => {
        draft.capsLockIndexes = collected;
      });
    } else {
      // reset if targetText is empty
      setStatistics(draft => {
        draft.capsLockIndexes = [];
      });
    }

  }, [targetText, setStatistics]);
