...
import { range } from '@/_shared/utils/range'
      ...
      <h1 className="leading-loose text-lg font-semibold text-gray-800 relative rounded-lg my-3">

        {
          ...
          targetText.split("").map((char, index) => {
            ...
            const lockedLen = lockedText.length;
            const inputLen = inputBoxValue.length;
            ...
            const nextWordRange = range(lockedLen, lockedLen + nextWordLen - 1);

            // Green for correct & locked text
            if (index < lockedLen || index < realTimeCorrectTextLen) {
              className = "text-green-500";

            // Red for current incorrect input
            } else if (index < lockedLen + inputLen) {
              className = "bg-red-300 text-white";
            }

            // Check if index is in next word range
            if (inputLen > 0 && nextWordRange.includes(index)) {
              const isCorrectSoFar = targetText.startsWith(lockedText + inputBoxValue);
              className = isCorrectSoFar
                ? "bg-green-200 text-green-300 blur-xs"
                : "bg-red-200 text-red-300 blur-xs";
            }
